...
def _create_centered_header(title: str, length: int = 80, char: str = "=") -> str:
    ...
    ----------
    title : str
                                                                                                                                    The title or name to be displayed in the center of
                                                                                                                                    the header.
    length : int, optional
                                                                                                                                    The total length of the header line (default is
                                                                                                                                    80).
    char : str, optional
    ...
    -------
    str
                                                                                                                                    A formatted string with the title centered and the
                                                                                                                                    separator character filling the
                                                                                                                                    rest of the
                                                                                                                                    line.
...
def data_heads(
    dataframes: Union[DataFrameList, DataFrameDict], separator_length: int = 80
) -> None:
    """
    ...
    dataframes : Union[DataFrameList, DataFrameDict]
                                                                                                                                    A list or dictionary of Pandas or Polars
...
def _validate_dataframes(df1: DataFrameType, df2: DataFrameType) -> None:
    ...
    if isinstance(df1, pd.DataFrame) and isinstance(df2, pd.DataFrame):
        ...
        raise ValueError(
            "Both dataframes must be of the same type (either Pandas or Polars)."
        )
...
def _get_numeric_columns_only(df: DataFrameType) -> DataFrameType:
    ...
    if isinstance(df, pd.DataFrame):
        ...
        numeric_df = df.select(
            pl.col(pl.Int8, pl.Int16, pl.Int32, pl.Int64, pl.Float32, pl.Float64)
        )
...
def compare_datasets(df1: DataFrameType, df2: DataFrameType) -> DataFrameType:
    ...
    if isinstance(df1, pd.DataFrame):
        ...
        comparison = desc_df1.with_columns(
            [
                (desc_df1.get_column(col) - desc_df2.get_column(col)).alias(col)
                for col in desc_df1.columns
            ]
        )
...
def compare_multiple_datasets(
    ...
    datasets : Union[List[pd.DataFrame, pl.DataFrame], Dict[str, pd.DataFrame, pl.DataFrame]]
                                                                                                                                    A list or dictionary of datasets to compare. If a
                                                                                                                                    dictionary is passed, the keys
                                                                                                                                    will be used as
                                                                                                                                    dataset
                                                                                                                                    names.
    ...
    ------
    ValueError
                                                                                                                                    If the datasets parameter is neither a list nor a
                                                                                                                                    dictionary.
...
def compare_means(df1: DataFrameType, df2: DataFrameType) -> DataFrameType:
    ...
    ----------
    df1 : DataFrameType
                                                                                                                                    The first dataset (pandas or polars DataFrame) to
                                                                                                                                    compare.
    df2 : DataFrameType
                                                                                                                                    The second dataset (pandas or polars DataFrame) to
                                                                                                                                    compare.

    Returns
    -------
    DataFrameType
                                                                                                                                    A DataFrame containing the differences in means
                                                                                                                                    for each column.
...
def perform_ttest(
    ...
    ----------
    df1 : DataFrameType
                                                                                                                                    The first dataset (Pandas or Polars DataFrame)
                                                                                                                                    containing numeric and other types
                                                                                                                                    of columns.
    df2 : DataFrameType
                                                                                                                                    The second dataset (Pandas or Polars DataFrame)
                                                                                                                                    containing numeric and other types
                                                                                                                                    of columns.
    ...
    ttest_results : dict
                                                                                                                                    ...
                                                                                                                                    containing
                                                                                                                                    the T-statistic ("t_stat") and the p-value (
                                                                                                                                    "p_val") for each numeric column
                                                                                                                                    comparison.
                                                                                                                                    Example:
                                                                                                                                    {
    ...
    if isinstance(df1, pd.DataFrame) and isinstance(df2, pd.DataFrame):
        ...
        for col in df1.columns:
            if df1[col].dtype in [pl.Float32, pl.Float64, pl.Int32, pl.Int64] and df2[
                col
            ].dtype in [pl.Float32, pl.Float64, pl.Int32, pl.Int64]:
                # Perform T-test on non-NaN values of the column (Polars to Pandas conversion for T-test)
                t_stat, p_val = stats.ttest_ind(
                    df1[col].drop_nulls().to_pandas(), df2[col].drop_nulls().to_pandas()
                )
                # Store the results in the dictionary
                ttest_results[col] = {"t_stat": t_stat, "p_val": p_val}
...
def filter_significant_results(
    ttest_results: Dict[str, Dict[str, float]], p_value_threshold: float = 0.05
) -> Dict[str, Dict[str, float]]:
    """
    ...
    ttest_results : dict
                                                                                                                                    ...
                                                                                                                                    and values are dicts containing 't_stat'
                                                                                                                                    and 'p_val'.
    ...
    -------
    dict
                                                                                                                                    A dictionary containing only the significant
                                                                                                                                    features where p-value <
                                                                                                                                    p_value_threshold.
    """
    significant_results = {
        col: stats
        for col, stats in ttest_results.items()
        if stats["p_val"] < p_value_threshold
    }
    return significant_results


def compare_multiple_datasets_significant(
    datasets: Union[List[pd.DataFrame], List[pl.DataFrame]],
    ...
    p_value_threshold : float, optional
                                                                                                                                    The p-value threshold for significance (default is
                                                                                                                                    0.05).

    Returns
    -------
    dict
                                                                                                                                    A dictionary of significant results for each pair
                                                                                                                                    of datasets.
                                                                                                                                    The format is { 'dataset_i_vs_dataset_j': {
                                                                                                                                    'feature': { 't_stat': float,
                                                                                                                                    'p_val': float }, ... } }
    """
    significant_results = {}
